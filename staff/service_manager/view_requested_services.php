    ...
    <div id="wrapper">
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card">
                                ...
                                <table class="table table-bordered" id="appointment">
                                    ...
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        "
                                           ...
                                           <td> <a href='view_employees.php?rq_id=$row[request_id]&&s_type=$row[service_name] 'class='btn btn-success'>Process<a>
                            ...
                            <?php
                            $get_user_id = $mysqli->query("SELECT * FROM service_manager where email='$user'");
                            $row = $get_user_id->fetch_assoc();
                            $u_id = $row['id'];

                            if (isset($_GET['ap_payment'])) {
                                $id = $_GET['ap_payment'];
                                $result = $mysqli->query("UPDATE payments SET payment_status='approved',finance_manager_id='$u_id' WHERE id = $id") or die($mysqli->error);


                                echo '<script>
        alert("approved successfully!")
        window.location.replace("view_payments.php");
        </script>';
                            }
                            if (isset($_GET['rj_payment'])) {
                                $id = $_GET['rj_payment'];
                                $result = $mysqli->query("UPDATE payments SET payment_status='rejected', set finance_manager_id='$u_id' WHERE id = $id") or die($mysqli->error);
       ...
       window.location.replace("view_payments.php")  
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Processed Cleaners History</h6>
                        </div>
                        <div class="card">
                            <div class="table-responsive">
                                <table class="table table-bordered" id="appointment">
                                    ...
                                    <th>Task Payment Status</th>
                                    <th> Task Status</th>



                                    <?php
                                    $result = $mysqli->query("SELECT  clients.firstname as firstname,clients.lastname as lastname,cleaner.firstname as cleaner_firstname,cleaner.lastname as cleaner_lastname,service_manager.firstname as service_manager_firstname,service_manager.lastname as service_manager_lastname,payments.payment_status as payment_status,requested_services.subscription_status as subscription_status FROM requested_services join clients on clients.id=requested_services.client_id join service_manager on service_manager.id=requested_services.service_manager_id join payments on requested_services.id=payments.request_id join cleaner on cleaner.id=requested_services.assigned_employee_id order by requested_services.id desc") or die($mysqli->error);
                                    while ($row = $result->fetch_assoc()) {
                                        echo
                                        "
                                            <tbody>
                                           ...
                                           <td>" . $row['firstname'] . "</td>
                                           <td>" . $row['lastname'] . "</td>
                                           <td>" . $row['cleaner_firstname'] . "</td>
                                           <td>" . $row['cleaner_lastname'] . "</td>
...
window.location.relace("viewstaff.php")  
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Processed Garbage Collection History</h6>
                        </div>
                        <div class="card">
                            <div class="table-responsive">
                                <table class="table table-bordered" id="appointment">
                                ...
                                <th>Client FirstName</th>
                                    ...
                                    <?php
                                    $result = $mysqli->query("SELECT  clients.firstname as firstname,clients.lastname as lastname,garbage_collectors.firstname as garbage_collector_firstname,garbage_collectors.lastname as garbage_collector_lastname,service_manager.firstname as service_manager_firstname,service_manager.lastname as service_manager_lastname,payments.payment_status as payment_status,requested_services.subscription_status as subscription_status FROM requested_services join clients on clients.id=requested_services.client_id join service_manager on service_manager.id=requested_services.service_manager_id join payments on requested_services.id=payments.request_id join garbage_collectors on garbage_collectors.id=requested_services.assigned_employee_id order by requested_services.id desc") or die($mysqli->error);
                                    while ($row = $result->fetch_assoc()) {
                                        echo
                                        "
                                            <tbody>
