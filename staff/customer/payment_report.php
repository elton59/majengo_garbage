...
function fetch_data()
{
  ...
  if(isset($_GET['uid']))
  {
   ...
   $u_id = $row['id'];
     $result=$mysqli->query("SELECT  clients.firstname as firstname,clients.lastname as lastname, payments.id as payment_id,payments.transaction_id as transaction_id,payments.amount as amount_paid,requested_services.service_name as service_name,payments.payment_date as payment_date,payments.payment_status as payment_status,requested_services.pickup_location as pickup_location from payments  join clients on clients.id=payments.userid join requested_services on payments.request_id=requested_services.id where clients.id= '$u_id'  order by payments.id desc") or die($mysqli->error);
