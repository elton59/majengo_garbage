                            ...
                            <table class="table table-bordered" id="appointment">
                                    ...
                                    <th>Status</th>
                                      ...
                                      $u_id = $row['id'];
                                        ...
                                        $result=$mysqli->query("SELECT  clients.firstname as firstname,clients.lastname as lastname, payments.id as payment_id,payments.transaction_id as transaction_id,payments.amount as amount_paid,payments.payment_date as payment_date,payments.payment_status as payment_status  from payments  join clients on clients.id=payments.userid where payments.payment_status='pending' and  clients.id= '$u_id'  order by payments.id desc") or die($mysqli->error);
...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">My Payment status</h6>
    </div>
    <div class="card">
        <div class="table-responsive"  >
        <table class="table table-bordered" id="appointment">
        ...
        <th>FirstName</th>
                                    ...
                                    <th>Status</th>
                                      ...
                                      $row = $get_user_id->fetch_assoc();
                                      $u_id = $row['id'];
                                        $result=$mysqli->query("SELECT  clients.firstname as firstname,clients.lastname as lastname, payments.id as payment_id,payments.transaction_id as transaction_id,payments.amount as amount_paid,payments.payment_date as payment_date,payments.payment_status as payment_status  from payments  join clients on clients.id=payments.userid where clients.id= '$u_id' order by payments.id desc") or die($mysqli->error);
                                        while($row=$result->fetch_assoc())
                                        {
                                            echo
                                            "
             ...
             <a target="_blank"  href="payment_report.php?uid=<?php echo $user?>"  button  class="btn btn-danger" >export to pdf</a>
